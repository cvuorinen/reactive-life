...
export class Patterns {
    ...
    static getAvailablePatterns(): Array<string> {
        return _.keys(Patterns.patterns);
    }

    static loadPattern(game: Game, patternName: string, colors: Array<string>) {
        if (!Patterns.patterns[patternName]) {
            return;
        }

        _(game.cells)
            .filter(cell => !cell.alive && cellInPattern(cell))
            .map(cell => cell.setAlive(_.sample(colors)))
            .value();

        function cellInPattern(cell: Cell) {
            return _.contains(
